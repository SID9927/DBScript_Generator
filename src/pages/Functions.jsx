...
const Functions = () => {
    ...
    const generateSingleScript = () => {
        ...
        setTimeout(() => {
            ...
            const backupName = `${funcName}_${actualSuffix}`;

            const sqlBackupScript = `
 IF EXISTS (SELECT * FROM sys.objects WHERE name = '${funcName}' AND type IN ('FN', 'IF', 'TF')) 
 AND NOT EXISTS (SELECT * FROM sys.objects WHERE name = '${backupName}' AND type IN ('FN', 'IF', 'TF')) 
 BEGIN
     EXEC sp_rename '${funcName}', '${backupName}';
 END
 GO`.trim();

            const sqlRollbackScript = `
 IF EXISTS (SELECT * FROM sys.objects WHERE name = '${backupName}' AND type IN ('FN', 'IF', 'TF')) 
 BEGIN
     IF EXISTS (SELECT * FROM sys.objects WHERE name = '${funcName}' AND type IN ('FN', 'IF', 'TF')) 
         DROP FUNCTION dbo.${funcName};
 
     EXEC sp_rename '${backupName}', '${funcName}';
 END
 GO`.trim();
    ...
    const generateBulkScripts = () => {
        ...
        setTimeout(() => {
            ...
            funcNames.forEach(name => {
                const backupName = `${name}_${actualSuffix}`;

                backupScripts.push(`
-- Backup: ${name}
IF EXISTS (SELECT * FROM sys.objects WHERE name = '${name}' AND type IN ('FN', 'IF', 'TF')) 
AND NOT EXISTS (SELECT * FROM sys.objects WHERE name = '${backupName}' AND type IN ('FN', 'IF', 'TF')) 
BEGIN
    EXEC sp_rename '${name}', '${backupName}';
END
GO`);
...
BEGIN
    ...
    EXEC sp_rename '${backupName}', '${name}';
END
GO`);
            });
    ...
    return (
        <>
            <SEO
                title="Function Backup"
                description="Safely generate backup and rollback scripts for SQL Server Functions. Supports bulk generation."
            />
            <SubNav />
            <div className="p-8 max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold mb-6 text-white">
                    Function Backup & Rollback
                </h1>

                <div className="bg-blue-900/20 border border-blue-900/50 p-4 mb-8 rounded-md">
                    <p className="text-sm text-blue-200">
                        <span className="font-semibold">How it works:</span> Backup renames existing function to *_suffix using <code className="bg-blue-900/50 px-1 rounded text-blue-100">sp_rename</code>.
                ...
                <div className="space-y-6">
                    {mode === 'single' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                            ...
                            <div className="md:col-span-2">
                                <p className="text-sm text-slate-500 mb-4">
                                    Backup Function will be named: <strong className="text-slate-300">{funcName || 'FunctionName'}_{suffix || defaultSuffix}</strong>
                                </p>
                                <div className="flex gap-3">
                                    <button
                                        className="btn-primary"
                                        onClick={generateSingleScript}
                                        disabled={!funcName || isGenerating}
                                    >
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-2">
                                    Function Names <span className="text-slate-500 font-normal">(One per line)</span>
                                </label>
                                <textarea
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-1">
                                    Same suffix will be applied to all functions. Default: {defaultSuffix}
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                                <div className="bg-blue-900/10 border border-blue-500/20 rounded-lg p-4">
                                    ...
                                    <div className="text-2xl font-bold text-slate-200">
                                        {mode === 'single' ? (funcName ? 1 : 0) : bulkInput.split('\n').filter(l => l.trim()).length}
                                    </div>
                                </div>
