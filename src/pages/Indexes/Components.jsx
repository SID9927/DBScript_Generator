...
export const CodeBlock = ({ children }) => {
    ...
    return (
        <div className="relative group/code my-4">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 rounded-2xl blur opacity-0 group-hover/code:opacity-100 transition duration-1000"></div>
            <div className="relative bg-slate-950/80 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm">
                ...
                <pre className="p-6 text-sm md:text-base font-mono leading-relaxed overflow-x-auto custom-scrollbar">
                    <code className="text-indigo-200">{children}</code>
                </pre>
            </div>
        </div>
    );
...
export const InfoCard = ({ type = "info", children }) => {
    ...
    return (
        <div className={`p-6 rounded-[2rem] border-2 border-dashed ${styles[type]} backdrop-blur-sm relative overflow-hidden group`}>
...
export const CustomSelect = ({ label, value, options, onChange }) => {
    ...
    return (
        <div className="relative space-y-1.5">
            <label className="text-[9px] font-black text-slate-600 tracking-[0.3em] mb-1 px-1 block italic lowercase first-letter:uppercase">
                {label}
            </label>
            ...
            <div className="relative">
                <button
                    ...
                    className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-2.5 text-indigo-300 font-bold text-xs tracking-tight text-left flex items-center justify-between group transition-all hover:border-indigo-500/40 relative overflow-hidden"
                >
                    <div className="absolute inset-0 bg-indigo-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <span className="relative z-10">{activeOption.label}</span>
                    <svg
                        className={`w-3.5 h-3.5 text-slate-600 transition-transform duration-300 relative z-10 ${isOpen ? 'rotate-180' : ''}`}
                ...
                {isOpen && (
                    <>
                        ...
                        <div className="absolute top-full left-0 w-full mt-1 bg-slate-900/95 border border-slate-800 rounded-xl overflow-hidden shadow-2xl backdrop-blur-xl z-[70] animate-in fade-in slide-in-from-top-1 duration-200">
                            <div className="max-h-60 overflow-y-auto custom-scrollbar">
                                {options.map((opt) => (
                                    <button
                                        ...
                                        className={`w-full px-4 py-2.5 text-left text-[11px] font-bold transition-all flex items-center justify-between border-b border-slate-800/50 last:border-0 ${value === opt.value
