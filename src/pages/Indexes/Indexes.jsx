...
const IndexesPage = () => {
    ...
    const tabs = [
        { id: 'guide', name: 'Strategic Guide', icon: '📚', sub: 'Optimization core' },
        { id: 'terms', name: 'Terms Dictionary', icon: '🧠', sub: 'Index Operator Logic' },
        { id: 'playground', name: 'Syntax Forge', icon: '🛠', sub: 'T-SQL Blueprinting' }
    ];

    return (
        <div className="p-6 max-w-7xl mx-auto space-y-8">
            <SEO 
                title="SQL Indexes Optimization | Developer Suite" 
                description="Master SQL Server index performance. Comprehensive guide to Clustered, Non-Clustered, and Filtered indexes."
            />
                ...
                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="space-y-1 pr-12">
                            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight uppercase italic leading-none">
                                SQL <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-indigo-600 pr-2">INDEXES</span>
                            </h1>
                            <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[8px] italic leading-tight">
                                High-performance <span className="text-indigo-500/80">data retrieval</span> architecture protocol
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 bg-slate-950/40 px-6 py-4 rounded-[1.8rem] border border-slate-800/40 backdrop-blur-md shadow-inner">
                        <div className="flex flex-col items-end">
                            <span className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mb-2 italic flex items-center gap-1.5 font-mono">
                                <span className="w-1 h-1 rounded-full bg-indigo-500 animate-pulse" />
                                Indexation Status
                            </span>
                            <div className="flex gap-1">
                                {[1, 2, 3, 4, 5, 6].map(i => (
                                    <div key={i} className={`w-1.5 h-4 rounded-sm skew-x-[-15deg] transition-all duration-500 ${i <= 5 ? 'bg-indigo-500' : 'bg-slate-800'}`} />
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col">
                            <div className="text-[7px] font-black text-slate-600 uppercase tracking-[0.2em] mb-0.5 whitespace-nowrap font-mono">Performance Grade</div>
                            <div className="flex items-baseline gap-0.5 leading-none">
                                <span className="text-xl font-black text-white tabular-nums">A</span>
                ...
                <div className="lg:w-80 w-full flex-shrink-0 lg:sticky lg:top-24 z-20">
                    <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-4 shadow-2xl space-y-2">
                            ...
                            {tabs.map((tab) => (
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className="text-[9px] font-bold text-slate-600 uppercase tracking-[0.2em] group-hover:text-indigo-400/60 transition-colors mt-1.5">{tab.sub}</div>
                                    </div>
                                    <AnimatePresence>
                                        {activeTab === tab.id && (
                                            <motion.div layoutId="indexTabPill" className="absolute inset-0 border-2 border-indigo-500/20 rounded-2xl shadow-[inset_0_0_20px_rgba(99,102,241,0.1)]" />
                                        )}
                                    </AnimatePresence>
                                </button>
                            ))}
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-800/50 px-2 pb-2">
                             <div className="bg-slate-950/50 rounded-2xl p-4 border border-slate-800/40">
                                 <div className="flex items-center gap-2 mb-3">
                                     <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                                     <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest font-mono">B-Tree Traversal</span>
                                 </div>
                                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter leading-tight italic">Root &gt; Intermediate &gt; Leaf</p>
                                 <p className="text-[8px] text-slate-600 font-medium uppercase tracking-widest mt-1">Validated Logic</p>
                             </div>
                        </div>
                    </div>
                </div>
                ...
                >
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {activeTab === 'terms' && <Terms terms={indexTerms} />}
