...
export const CodeBlock = ({ children }) => {
    ...
    return (
        <div className="relative group/code my-4">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-2xl blur opacity-0 group-hover/code:opacity-100 transition duration-1000"></div>
            <div className="relative bg-slate-950/80 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm">
                ...
                <pre className="p-6 text-[13px] md:text-sm font-mono leading-relaxed overflow-x-auto custom-scrollbar">
                    <code className="text-emerald-200">{children}</code>
                </pre>
            </div>
        </div>
    );
        ...
        <div className="relative space-y-1.5">
            ...
            <div className="relative">
                <button
                    ...
                    className={`w-full bg-slate-950/50 border border-slate-800 rounded-xl transition-all hover:border-emerald-500/40 relative overflow-hidden text-left flex items-center justify-between group ${compact ? 'px-3 py-1.5 text-[10px]' : 'px-4 py-2.5 text-xs'} font-bold text-emerald-400 tracking-tight`}
                >
