    ...
    return (
        <div className="space-y-4">
            {terms.map((term, index) => (
                <div
                    key={index}
                    className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden transition-all duration-300 hover:border-sky-500/30"
                >
                    <div
                        className="p-6 flex items-center justify-between cursor-pointer hover:bg-slate-800/30 transition-colors group"
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4">
                             <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-colors ${
                                ...
                                term.color === 'sky' ? 'bg-sky-500/10 text-sky-400 border-sky-500/30' :
                                term.color === 'amber' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30' :
                                term.color === 'red' ? 'bg-red-500/10 text-red-400 border-red-500/30' :
                                'bg-slate-800 text-slate-400 border-slate-700'
                             }`}>
                                {term.impact}
                             </div>
                             <div className={`w-8 h-8 rounded-full border border-slate-800 flex items-center justify-center text-slate-600 transition-transform duration-500 ${expandedTerm === index ? 'rotate-180 bg-sky-500/20 text-sky-400 border-sky-500/30' : ''}`}>
                    ...
                    <AnimatePresence>
                        {expandedTerm === index && (
                            ...
                            >
                                <div className="p-8 bg-slate-950/30 grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                                    <div className="space-y-8">
                                        <div className="space-y-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-2 h-2 rounded-full bg-sky-500" />
                                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Fundamental Concept</h4>
                                            </div>
                                    ...
                                    <div className="space-y-8">
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-xs font-bold text-sky-400 leading-relaxed italic opacity-80">{term.scenario}</p>
                                        </div>

                                        <div className="space-y-4">
                                             <div className="flex items-center gap-3">
                                                <svg className="w-3.5 h-3.5 text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
