...
export const CodeBlock = ({ children }) => {
    ...
    return (
        <div className="relative group/code my-4">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-sky-500/20 to-cyan-500/20 rounded-2xl blur opacity-0 group-hover/code:opacity-100 transition duration-1000"></div>
            <div className="relative bg-slate-950/80 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm">
                ...
                <pre className="p-6 text-[13px] md:text-sm font-mono leading-relaxed overflow-x-auto custom-scrollbar">
                    <code className="text-sky-200">{children}</code>
                </pre>
            </div>
        </div>
    );
        ...
        <div className="relative space-y-1.5 text-left">
            ...
            <div className="relative">
                <button
                    ...
                    className={`w-full bg-slate-950/50 border border-slate-800 rounded-xl transition-all hover:border-sky-500/40 relative overflow-hidden text-left flex items-center justify-between group ${compact ? 'px-3 py-1.5 text-[10px]' : 'px-4 py-2.5 text-xs'} font-bold text-sky-400 tracking-tight`}
                >
                                ...
                                {options.map((opt) => (
                                    <button
                                        ...
                                        className={`w-full text-left font-bold transition-all flex items-center justify-between border-b border-slate-800/50 last:border-0 ${compact ? 'px-3 py-1.5 text-[9px]' : 'px-4 py-2.5 text-[11px]'} ${
                                            value === opt.value 
                                                ? 'bg-sky-600/10 text-white' 
                                                : 'text-slate-400 hover:bg-slate-800 hover:text-sky-400'
                                        }`}
                                    >
                                        <span className="truncate">{opt.label}</span>
                                        {value === opt.value && (
                                            <div className="w-1.5 h-1.5 rounded-full bg-sky-500 shadow-[0_0_8px_rgba(14,165,233,0.6)] shrink-0" />
                                        )}
                                    </button>
                                ))}
                            </div>
